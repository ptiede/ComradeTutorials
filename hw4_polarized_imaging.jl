...
The publically released EHT data has been converted to circular polarization, so for this tutorial we will stick with that.
"""
...
!!! note
	...
	Although we are limited by what uvfits and eht-imaging support. If you want to fit linear feeds and/or mixed polarization please get in touch with Paul Tiede.
"""
...
obs = Pyehtim.load_uvfits_and_array(
        "Data/m87polarized.uvfits",
        "Data/array.txt", polrep="circ")
...
dvis = extract_table(obsavg, Coherencies())
...
J = adjoint(R)*G*D*R,
```
where we need to apply the adjoint
...
begin
	...
	figbl = Figure(;size=(800, 600))
	baselineplot(figbl[1,1], res[1], :uvdist, x->Comrade.measurement(x)[1,1]/noise(x)[1,1], axis=(ylabel="RR Residual", xlabel="uv distance (λ)"))
	baselineplot(figbl[2,1], res[1], :uvdist, x->Comrade.measurement(x)[2,1]/noise(x)[1,1], axis=(ylabel="LR Residual", xlabel="uv distance (λ)"))
	baselineplot(figbl[1,2], res[1], :uvdist, x->Comrade.measurement(x)[1,2]/noise(x)[1,1], axis=(ylabel="RL Residual", xlabel="uv distance (λ)"))
	baselineplot(figbl[2,2], res[1], :uvdist, x->Comrade.measurement(x)[2,2]/noise(x)[1,1], axis=(ylabel="LL Residual", xlabel="uv distance (λ)"))
	figbl
end
...
md"""
Now we optimize to find the MAP image. 
"""
...
begin
	gpl = imagepixels(μas2rad(150.0), μas2rad(150.0), 128, 128)
	img = intensitymap(Comrade.skymodel(post, xopt), gpl)
	#Plotting the results gives
	imageviz(img, adjust_length=true, colormap=:bone, pcolormap=:RdBu)
end

# ╔═╡ 387318ce-e7b2-11ef-3a18-39f0dc312400
md"""
!!! note
	The image looks a little noisy. This is an artifact of the MAP image. To get a publication quality image we recommend sampling from the posterior and averaging the samples. See homework question 1.
"""
...
begin
	...
	plotcaltable(gphaseR, gphase_ratio, labels=["R Phase", "L/R Phase"]);
end
...
begin
	...
	plotcaltable(gampr, gamp_ratio, labels=["R Amp", "L/R Amp"], axis_kwargs=(;limits=(nothing, (0.6, 1.3))));
end

# ╔═╡ a73b478e-dc4a-467d-a16d-9f6da4fb5c1d
md"""
### Homework
 1. Sample the image posterior using NUTSs. The previous tutorials should show you how to do this.
 2. There are different parameterizations possible for polarized imaging. One problem with the Poincare representation is that angles make it hard to define correlations in the polarized maps Q, U, V. Try reading the documentation of `PolExp2Map` and see if you can adjust the sky prior and model function to use this alternative representation.
 3. Like the total intensity models, we can define multiple components for our polarized sky model. Look at the documentation for `PolarizedModel` and see if you can create an additional large scale Gaussian component to include in the sky model.  
"""
...
manifest_format = "2.0"
project_hash = "9e0cbbfd68337f6a2e4aa3e3a13c6f2b2712e47e"
