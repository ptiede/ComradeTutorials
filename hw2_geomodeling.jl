...
# ╔═╡ 2fcb8fda-e7b2-11ef-22ae-bffecff16401
using Comrade

# ╔═╡ 2fcb8fec-e7b2-11ef-1f1e-9742df5f49ed
using Pyehtim

# ╔═╡ 2fcb900c-e7b2-11ef-01ee-35115b59dc8b
begin
	using StableRNGs
	rng = StableRNG(42)
end

# ╔═╡ 2fcb90b6-e7b2-11ef-3b83-09de3bc6716c
begin
...
# ╔═╡ 14438b12-1c36-4274-b9ee-95acf42b8a86
using CairoMakie

# ╔═╡ 2fcb91c6-e7b2-11ef-2b60-4b58a703879a
begin
	using Optimization
	using OptimizationBBO
end

# ╔═╡ 22c523e0-a1cf-4e08-a517-177a5c8b2382
using Pigeons

# ╔═╡ c48ef5cc-3d19-45ed-9bb5-fdacfeb838b4
using MCMCDiagnosticTools

# ╔═╡ 8d3d1ade-556a-4b6e-8efa-09d87327a53a
using PairPlots

# ╔═╡ 2fcb8f94-e7b2-11ef-1784-1d0dc3c6e7b5
md"# Geometric Modeling of EHT Data"

# ╔═╡ 2fcb8fa8-e7b2-11ef-14d1-2be73dac8760
md"""
...
# ╔═╡ 2fcb8fba-e7b2-11ef-0d10-6d4e329220e2
md"""
...
# ╔═╡ 2fcb8fc6-e7b2-11ef-1e08-7bf85100ea60
md"To get started we load Comrade."

# ╔═╡ 2fcb8fe4-e7b2-11ef-007e-87526f012b97
md"""
Currently we use eht-imaging for data management, however this will soon be replaced
by a pure Julia solution.
"""

# ╔═╡ 2fcb9002-e7b2-11ef-2dbf-f9ff9ef1f063
md"For reproducibility we use a stable random number genreator"

# ╔═╡ 2fcb9016-e7b2-11ef-2835-e7e618121f6d
md"""
...
For an introduction to data loading, see Loading Data into Comrade.
"""

# ╔═╡ a67e6fcb-c54a-43ae-afb3-35a59ee48aff
md"""
#### Question 1
> Load the data using Pyehtim, assuming we won't be using polarized data and save it to the variable `obs`
!!! hint
	See the previous tutorial hw1_loadingdata.jl` for an example
"""

# ╔═╡ 2fcb9020-e7b2-11ef-0b27-4f0b8c361bd3
# Load data here
obs = ehtim.obsdata.load_uvfits(joinpath(@__DIR__, "Data", "SR2_M87_2017_096_hi_hops_ALMArot.uvfits"))

# ╔═╡ 2fcb9034-e7b2-11ef-1572-e56b6dcf1dce
md"""
Now we will kill zero baselines since we don't care about large-scale flux.
If you want to fit the zero-baseline flux, you will need to add a large-scale Gaussian (see homework question 1 below).

To modify the data we currently use Pyehtim and eht-imaging's functions
"""

# ╔═╡ 2fcb9034-e7b2-11ef-1fb8-75bc2ce6b086
obsavg = Pyehtim.scan_average(obs.flag_uvdist(uv_min=0.1e9)).add_fractional_noise(0.02)

# ╔═╡ 2fcb9048-e7b2-11ef-105a-9151426cdff5
md"""
For this tutorial we will fit closure products for simplicity. Additionally,
adding gains typically requires a lot more parameters and can take a while when
not using a gradient sampler as we do below.

"""

# ╔═╡ 2fcb9052-e7b2-11ef-143a-f5f504622d23
dlcamp, dcphase = extract_table(obsavg, LogClosureAmplitudes(;snrcut=3.0), ClosurePhases(;snrcut=3.0))

# ╔═╡ 2fcb9066-e7b2-11ef-2452-ff788fb3a57f
md"""
!!! warn
   	We remove the low-snr closures since they are very non-gaussian. This can create rather large biases in the model fitting since the likelihood has much heavier tails that the usual Gaussian approximation.
"""

# ╔═╡ 2fcb9070-e7b2-11ef-0538-2f3617a2269d
md"""
...
For Comrade to specify a `SkyModel`, we need to define a function that accepts two arguments. The first argument `θ` is a NamedTuple of parameters. The second argument `p` is a NamedTuple or Tuple of fixed arguments needed to define the model fully.
For geometric models we don't need `p`.

For our model we will use a mring model plus a single *nuisance* Gaussian.
"""

# ╔═╡ 2fcb9084-e7b2-11ef-04ca-655199edb29f
function sky(θ, p)
    (;r, w, ma, mp, τ, ξτ, f, 
	  σg, τg, ξg, xg, yg) = θ
    α = ma.*cos.(mp .- ξτ)
...
# ╔═╡ 8dd34062-e321-4762-af52-8e2ea695ad68


# ╔═╡ e8798379-401a-4bd1-b99c-7e6cca8ce603
md"""
!!! note
	Julia's editor has unicode support. To create greek letter characters you can use the typical LaTeX code e.g. \alpha and then press TAB to autocomplete it into the correct unicode character.
"""

# ╔═╡ 2fcb90a2-e7b2-11ef-1c67-a9640f5c4b77
md"""
In addition to the model and parameterization, we need to specify the priors for our model parameters. Comrade works on `NamedTuple` so that the parameter order doesn't matter. To specify the prior, we just need to make sure every parameter in `θ` in `sky` 
"""

# ╔═╡ 2fcb90ca-e7b2-11ef-0474-13e02f972920
md"""
...
# ╔═╡ faa407b3-159d-42ff-828a-a182aa029630
md"""
To interact with the posterior let's first draw a random sample from the prior
"""

# ╔═╡ 5758d69b-2538-4d81-ba77-0d6a8f38f740
s = prior_sample(post)

# ╔═╡ 2fcb9110-e7b2-11ef-0929-4db55eaa7766
md"""
We can evaluate the unnormalized log posterior density using `logdensityof`
"""

# ╔═╡ 2fcb911a-e7b2-11ef-32e4-918ecdfcfc43
logdensityof(post, s)

# ╔═╡ 23c090fa-0515-4789-b5c3-a736a300f9eb
md"""
We can also create the skymodel for the prior draw using `skymodel`
"""

# ╔═╡ dc2d3797-715b-4ce4-b342-6662d916d37d
m = skymodel(post, s)

# ╔═╡ 16a6c75c-480b-45df-84de-14a3178a5775
md"""
And plot it using `imageviz`
"""

# ╔═╡ 99eee2b5-07bc-4483-b543-adb6ae5f0904
imageviz(intensitymap(m, skym.grid)) #grab the grid from skym

# ╔═╡ 2fcb9122-e7b2-11ef-31fe-8162640f0f94
md"## Reconstruction"

# ╔═╡ 2fcb9138-e7b2-11ef-3499-3b1ed247c8fd
md"""
...
# ╔═╡ 2fcb9142-e7b2-11ef-3604-b7e0ece066f1
md"""
...
# ╔═╡ 2fcb914c-e7b2-11ef-16cd-bd4b6020bff5
cpost = ascube(post)

# ╔═╡ 2fcb9154-e7b2-11ef-3232-edf6f991c446
md"""
If we want to flatten the parameter space and move from constrained parameters to (-∞, ∞)
support we can use the `asflat` function
"""

# ╔═╡ 2fcb9160-e7b2-11ef-17f5-3f386b951a13
fpost = asflat(post)

# ╔═╡ 2fcb916a-e7b2-11ef-0995-b18ef6c309a3
md"""
...
# ╔═╡ 2fcb9188-e7b2-11ef-141f-df92513feb26
md"and then transform it to transformed space using TransformVariables"

# ╔═╡ 2fcb9192-e7b2-11ef-3213-bb633b01c49a
logdensityof(fpost, Comrade.inverse(fpost, s))

# ╔═╡ 2d5237e6-922f-4c0e-a087-5a1e0c6abc1d
prior_sample(fpost)

# ╔═╡ 2fcb919c-e7b2-11ef-0547-e75f3009ae2c
md"""
!!! note
	The log density is not the same as above because the parameter transformation causes a jacobian term to appear to match the change in prior volume.
"""

# ╔═╡ 2fcb91a6-e7b2-11ef-10e5-41e0e8db8cdf
md"### Finding the Optimal Image"

# ╔═╡ 2fcb91ba-e7b2-11ef-04eb-3706b1a66ce5
md"""
...
specifically the [`BlackBoxOptim.jl`](https://github.com/robertfeldt/BlackBoxOptim.jl) package.
"""

# ╔═╡ 32f932a7-e4f1-4c44-863e-3f253bc14a8d
md"""
To optimize the posterior, i.e. find the MAP estimate we use the `comrade_opt` function.
"""

# ╔═╡ 86a6fb0c-8f1e-449e-9d72-6e9764bcc943
xopt, sol = comrade_opt(post, BBO_adaptive_de_rand_1_bin_radiuslimited(); maxiters=50_000);

# ╔═╡ 2fcb91ce-e7b2-11ef-379c-355fdbd21df8
md"Given this we can now plot the optimal image or the *maximum a posteriori* (MAP) image."

# ╔═╡ 2fcb91e2-e7b2-11ef-14ab-091f5a17aa1d
imageviz(intensitymap(skymodel(post, xopt), skym.grid), colormap=:afmhot, size=(500, 400))

# ╔═╡ 2fcb91ec-e7b2-11ef-04dd-4bb5ce6f8375
md"### Quantifying the Uncertainty of the Reconstruction"

# ╔═╡ 2fcb9200-e7b2-11ef-0416-a7ac7273100b
md"""
...
# ╔═╡ 13ea6bc3-4506-4a7c-a93a-73d24ae7e017
Threads.nthreads()

# ╔═╡ 2fcb920a-e7b2-11ef-14c9-7bc80f2e66b8
pt = pigeons(target=cpost, explorer=SliceSampler(), record=[traces, round_trip, log_sum_ratio], n_chains=16, n_rounds=10, multithreaded=true)

# ╔═╡ 2fcb921e-e7b2-11ef-384d-b9b7f7809fd9
md"""
...
# ╔═╡ 2fcb922a-e7b2-11ef-10df-5921da3c5a5a
chain = sample_array(cpost, pt)

# ╔═╡ 2fcb923c-e7b2-11ef-1b7b-cf4c4878653a
md"First to plot the image we call"

# ╔═╡ 2fcb9246-e7b2-11ef-2d7e-17f3b2bd9b34
imgs = intensitymap.(skymodel.(Ref(post), sample(chain, 100)), Ref(skym.grid))

# ╔═╡ 2ac18331-5c10-4b92-8ff0-311b1467fb04


# ╔═╡ 3797dd7e-6e97-4fd9-932d-acc51a67c372
imageviz(imgs[40])

# ╔═╡ 2fcb9250-e7b2-11ef-39e7-85041294e7d1
md"""
What about the mean image? Well let's grab 100 images from the chain.
"""

# ╔═╡ 2fcb9264-e7b2-11ef-0fab-7df935f803f6
imageviz(mean(imgs))

# ╔═╡ 2fcb926e-e7b2-11ef-122b-3dabae46a3f8
md"""
...
# ╔═╡ 2fcb92a0-e7b2-11ef-2ab1-45c77a2efa39
begin
...
# ╔═╡ 2fcb92aa-e7b2-11ef-35e0-95bd39a838fc
md"""
...
# ╔═╡ 2fcb92bc-e7b2-11ef-0b6a-7122b1f7b7c2
begin
	fig2 = Figure(;size=(800, 300))
	ax12 = Axis(fig2[1,1], xlabel="√Quadrangle Area", ylabel="Log Closure Amplitude")
	ax22 = Axis(fig2[1,2], xlabel="√Triangle Area", ylabel="Closure Phase")
	for i in 1:10
	    mobs = simulate_observation(post, sample(chain, 1)[1])
	    mlca = mobs[1]
	    mcp  = mobs[2]
	    baselineplot!(ax12, mlca, uvdist, measurement, color=:grey, alpha=0.2)
	    baselineplot!(ax22, mcp, uvdist, mod2pi∘measurement, color=:grey, alpha=0.2)
	end
	baselineplot!(ax12, dlcamp, uvdist, measurement, marker=:x,)
	baselineplot!(ax22, dcphase, uvdist, mod2pi∘measurement, marker=:x,)
	fig2
end

# ╔═╡ 2fcb92c8-e7b2-11ef-13f6-53072b0b296d
md"""
...
# ╔═╡ 2fcb92dc-e7b2-11ef-0af0-df93d3169437
begin
	rd = residuals(post, chain[end])
	figres = Figure(;size=(800, 300))
	axisfields(figres[1,1], rd[1], uvdist, :res)
	axisfields(figres[1,2], rd[2], uvdist, :res)
	figres
end

# ╔═╡ ad0c88ef-10a8-4142-9743-7466459562cf
md"""
### Homework
1. Compare the results to the original EHT 2017 results. Are they the same or different? Try plotting the chain in a corner plot using [`PairPlots.jl`](https://github.com/sefffal/PairPlots.jl). If they are different, are we using a similar model?
2. Try fitting the zero-baseline flux: 
  - Reload the data so that the zbl points arent't flagged
  - Change the `sky` and `prior` definitions to include a large-scale Gaussian as an additional component and fit it. **Warning** keep in mind that total flux is not constrained by closures
  - The rest of the notebook should auto generate. 
  - What is the fractional of the image flux that is in the large-scale Gaussian.
  - Do the parameter estimates for the ring change, e.g., sharpen?
"""

# ╔═╡ 2fcb92e6-e7b2-11ef-33ca-434644deb3a1
md"""
---
...
manifest_format = "2.0"
project_hash = "b93a807e5af806e4130b7d6eda77931259c24a51"
...
[[deps.Comrade]]
deps = ["AbstractMCMC", "Accessors", "ArgCheck", "AstroTime", "ChainRulesCore", "ComradeBase", "DelimitedFiles", "DensityInterface", "DimensionalData", "Distributions", "DocStringExtensions", "EnzymeCore", "FillArrays", "ForwardDiff", "HypercubeTransform", "IntervalSets", "LinearAlgebra", "LogDensityProblems", "NamedTupleTools", "PaddedViews", "ParameterHandling", "PolarizedTypes", "PrettyTables", "Printf", "Random", "RecipesBase", "Reexport", "Serialization", "SparseArrays", "SpecialFunctions", "StaticArraysCore", "Statistics", "StatsBase", "StructArrays", "Tables", "TransformVariables", "VLBIImagePriors", "VLBILikelihoods", "VLBISkyModels"]
git-tree-sha1 = "01f4e47416d42ae0cbe93a7d48968be9f914a68f"
...
[[deps.ComradeBase]]
deps = ["DimensionalData", "DocStringExtensions", "EnzymeCore", "PolarizedTypes", "PrecompileTools", "Reexport", "StaticArrays", "StructArrays"]
git-tree-sha1 = "0c1e30fa4b0e8cf39732b265d8273c3d1dd2a9b7"
...
git-tree-sha1 = "5926fb95d5a4f3b2faef1b897239aa9dac870b08"
...
[[deps.Pigeons]]
deps = ["DataFrames", "Dates", "Distributions", "DocStringExtensions", "Expect", "FillArrays", "Graphs", "Interpolations", "JSON", "LinearAlgebra", "LogDensityProblems", "LogDensityProblemsAD", "LogExpFunctions", "MPI", "MPIPreferences", "MacroTools", "OnlineStatsBase", "OrderedCollections", "Pkg", "Printf", "Random", "RecipesBase", "Requires", "Roots", "Serialization", "SpecialFunctions", "SplittableRandoms", "StaticArraysCore", "Statistics", "StatsBase", "ZipFile"]
git-tree-sha1 = "71fc26715864c7c6481498fed22d699262c93fc3"
...
git-tree-sha1 = "e508b00334fc30f8cc45a2ec13f19aa85ef2fa89"
...
deps = ["ArgCheck", "DocStringExtensions", "ForwardDiff", "LinearAlgebra", "LogExpFunctions", "Random", "StaticArrays"]
git-tree-sha1 = "a3f06a3a66603dd69312ba4f77a45520df310895"
uuid = "84d833dd-6860-57f9-a1a7-6da5db126cff"
version = "0.8.13"

    [deps.TransformVariables.extensions]
    ChangesOfVariablesExt = "ChangesOfVariables"
    InverseFunctionsExt = "InverseFunctions"

    [deps.TransformVariables.weakdeps]
    ChangesOfVariables = "9e997f8a-9a97-42d5-a9f1-ce6bfc15e2c0"
    InverseFunctions = "3587e190-3f89-42d0-90ee-14403ec27112"
...
deps = ["AbstractFFTs", "ArgCheck", "Bessels", "ChainRulesCore", "ComradeBase", "DensityInterface", "Distributions", "DocStringExtensions", "EnzymeCore", "FFTW", "FillArrays", "HypercubeTransform", "LinearAlgebra", "NamedTupleTools", "Random", "Reexport", "Serialization", "SparseArrays", "SpecialFunctions", "StatsBase", "StatsFuns", "SuiteSparse", "TransformVariables"]
git-tree-sha1 = "6504d920fcbc65b48bc1b4d46883572e5ea0e967"
uuid = "b1ba175b-8447-452c-b961-7db2d6f7a029"
version = "0.9.4"

    [deps.VLBIImagePriors.extensions]
    VLBIImagePriorsEnzymeExt = "Enzyme"

    [deps.VLBIImagePriors.weakdeps]
    Enzyme = "7da242da-08ed-463a-9acd-ee780be4f1d9"

[[deps.VLBILikelihoods]]
deps = ["Bessels", "ChainRulesCore", "ComradeBase", "DensityInterface", "Distributions", "DocStringExtensions", "EnzymeCore", "LinearAlgebra", "Random", "Serialization", "SparseArrays", "StaticArraysCore", "StructArrays", "SuiteSparse"]
git-tree-sha1 = "1cb80cd6e236bcf5a962dc2e3dc93610815389f2"
uuid = "90db92cd-0007-4c0a-8e51-dbf0782ce592"
version = "0.2.8"

[[deps.VLBISkyModels]]
deps = ["AbstractFFTs", "Accessors", "ArgCheck", "ChainRulesCore", "ComradeBase", "DelimitedFiles", "DimensionalData", "DocStringExtensions", "EnzymeCore", "FFTW", "FITSIO", "FillArrays", "ForwardDiff", "GridInterpolations", "LinearAlgebra", "NFFT", "NamedTupleTools", "PaddedViews", "PolarizedTypes", "Printf", "RecipesBase", "Reexport", "Serialization", "SpecialFunctions", "StaticArrays", "StructArrays"]
git-tree-sha1 = "ccbf9939a36076e55996b917a053ccc39f0c5c86"
