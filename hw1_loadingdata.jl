...
# ╔═╡ 2adfdd78-e7b2-11ef-12bf-1bd678eabca6
begin
	using Comrade
	using CairoMakie
end

# ╔═╡ 2adfdda0-e7b2-11ef-3911-133f3fe5ab9d
using Pyehtim

# ╔═╡ a8b5d7bb-256b-4999-924c-a29c154068ab
using PlutoUI

# ╔═╡ 2adfdcc4-e7b2-11ef-1cf5-5dc50a05aca5
md"# Loading Data into Comrade"

# ╔═╡ 2adfdd1e-e7b2-11ef-16be-c3133c72d2df
md"""
...
# ╔═╡ 2adfdd3c-e7b2-11ef-0e4c-018c10d5efaf
md"""
...
# ╔═╡ 2adfdd50-e7b2-11ef-3561-45b6af227382
md"To get started, we will load `Comrade` and `Plots` to enable visualizations of the data"

# ╔═╡ 2adfdd8c-e7b2-11ef-3767-31ab40dca59f
md"""
We also load Pyehtim since it loads eht-imaging into Julia using PythonCall and exports
the variable ehtim
"""

# ╔═╡ 2adfddb4-e7b2-11ef-13d5-9994875ab6ad
md"""
...
# ╔═╡ 2adfddf0-e7b2-11ef-3fdd-7559c3c3c7b9
obsavg = Pyehtim.scan_average(obseht)

# ╔═╡ 2adfddfa-e7b2-11ef-1e52-cbbf11810bbe
md"""
> **Warning**
>
...
# ╔═╡ 2adfde0e-e7b2-11ef-20eb-870f22d621a7
md"We can now extract data products that `Comrade` can use"

# ╔═╡ 2adfde2c-e7b2-11ef-1b02-d1ab29d190ab
begin
	vis    = extract_table(obsavg, Visibilities()) ## complex visibilites
	amp    = extract_table(obsavg, VisibilityAmplitudes()) ## visibility amplitudes
	cphase = extract_table(obsavg, ClosurePhases(; snrcut=3.0)) ## extract minimal set of closure phases
	lcamp  = extract_table(obsavg, LogClosureAmplitudes(; snrcut=3.0)) ## extract minimal set of log-closure amplitudes
end

# ╔═╡ 2adfde40-e7b2-11ef-0d77-374ebe82c585
md"""
For polarized data, we need to be a little more careful. By default, eht-imaging will convert polarized data to the Stokes basis. If you call `obs.switch_polrep("circ")`, eht-imaging will not reproduce the circularly polarized data since it would need to track correlations between the data products.

To fix this `Pyehtim` a Julia wrapper of eht-imaging provides a special function for importing polarized data `load_uvfits_and_array` which will load the original circularly polarized data and an eht-imaging array table or `tarr` which is required to load the proper telescope mounts, which are critical for polarized imaging to work correctly.

!!! warn
	Failure to load the proper array file will lead to wrong results since the feed rotation Jones matrix will not be correct.
"""

# ╔═╡ 2adfde54-e7b2-11ef-0d65-4b21eb9627f0
begin
	obspol = Pyehtim.load_uvfits_and_array(
	                joinpath(@__DIR__, "Data", "m87polarized.uvfits"),
	                joinpath(@__DIR__, "Data", "array.txt"),
	                polrep="circ"
	                )
	obspolavg = Pyehtim.scan_average(obspol)
...
md"""
!!! warn
	This set of closures will look different from those produced directly from eht-imaging. eht-imaging does not produce the correct set of closures for a non-fully connected array. `Comrade` follows the procedure from Blackburn et al. (2020)
"""

# ╔═╡ 2adfde72-e7b2-11ef-3f0e-6b6bbbbc5ab2
md"""
To plot the uv-coverage we can use the simple forward facing function `plotfields`.

!!! hint
	As a reminder users can check the docstrings by typing `?` in the REPL, or using Pluto's live docs tab in the bottom right.
"""
...
begin
	figd = Figure(;size=(800, 600))
	axisfields(figd[1,1], vis, :uvdist, :measurement)
	axisfields(figd[1,2], amp, :uvdist, :measurement)
	axisfields(figd[2,1], cphase, :uvdist, :measurement)
	axisfields(figd[2,2], lcamp, :uvdist, :measurement)
	figd 
end

# ╔═╡ 2adfdeb8-e7b2-11ef-3b5e-41ed6314c635
md"And also the coherency matrices. Here we show how to use `baselineplot` to plot the data"

# ╔═╡ 2adfdeca-e7b2-11ef-0975-9126bf907743
begin
	figpol = Figure(;size=(800, 600))
	baselineplot(figpol[1,1], coh, x->uvdist(x)/1e9, x->abs(measwnoise(x)[1,1]), error=true, axis=(ylabel="RR", xlabel="uv distance (Gλ)"))
	baselineplot(figpol[2,1], coh, x->uvdist(x)/1e9, x->abs(measwnoise(x)[2,1]), error=true, axis=(ylabel="LR", xlabel="uv distance (Gλ)"))
	baselineplot(figpol[1,2], coh, x->uvdist(x)/1e9, x->abs(measwnoise(x)[1,2]), error=true, axis=(ylabel="RL", xlabel="uv distance (Gλ)"))
	baselineplot(figpol[2,2], coh, x->uvdist(x)/1e9, x->abs(measwnoise(x)[2,2]), error=true, axis=(ylabel="LL", xlabel="uv distance (Gλ)"))
	figpol
end

# ╔═╡ 19aa457a-7a05-49c6-8509-6dfe1e8e141e
md"""
You can also plot individual baselines using `baselineplot` or plotfields. For instance we can scroll through the amplitudes for each baseline
"""
...
begin
	fig, ax = baselineplot(vis, (s1, s2), :Ti, abs∘measwnoise, error=true, label="$s1 $s2")
	ax.xlabel = "Time (hours)"
	ax.ylabel = "Visibility Amplitude"
	axislegend(ax)
	fig
end
...
md"""
### Homework
 1. Try plotting other quantities with plotfields for specific baselines. Specifically `baselineplot` and `plotfields` can take any function for the plotting fields. If you use `measwnoise` for the first part you can also automatically do linear error propagation.
 2. For more ways to interact with the data directly check out [PlutoUI](https://featured.plutojl.org/basic/plutoui.jl)
"""
...
manifest_format = "2.0"
project_hash = "75a585ec04e809956927bf8aab22861ea6aed004"
