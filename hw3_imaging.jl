...
# ╔═╡ 3445bd92-e7b2-11ef-1e97-73adc8d22b72
using Comrade

# ╔═╡ 88d83445-2695-4ca2-b3ac-c0b5c2875beb
using Pyehtim


# ╔═╡ 3445bda6-e7b2-11ef-1675-05836b28be74
begin
	using StableRNGs
	rng = StableRNG(12)
end

# ╔═╡ 3445bd56-e7b2-11ef-1764-897407caf2b9
md"# Stokes I Simultaneous Image and Instrument Modeling"

# ╔═╡ 3445bd68-e7b2-11ef-051c-09e9f3c91d8f
md"""
...
# ╔═╡ 3445bd7e-e7b2-11ef-1121-7dc374e7a8f5
md"To get started we load Comrade."

# ╔═╡ 3445bd9c-e7b2-11ef-295f-b72902248fbc
md"For reproducibility we use a stable random number genreator"

# ╔═╡ 3445bdb0-e7b2-11ef-24d9-3b9affeaf09e
md"## Load the Data"

# ╔═╡ 3445bdba-e7b2-11ef-22d4-85cc63b5ed2f
md"""

#### Exercise 1
Load the data you want to analyze. We recommend the same data as in the previous examples as a starting point.

!!! hint
	Look at the previous tutorials for an example.
"""
...
md"""
Now you should do some minor processing
  - Scan average the data since the data have been preprocessed so that the gain phases
     coherent.
  - Add 1% systematic noise to deal with calibration issues that cause 1% non-closing errors.

!!! hint
	Check the previous tutorials for example code
"""

# ╔═╡ 3445bde2-e7b2-11ef-17f0-e1f1c1c65a2c
obsavg = scan_average(obs).add_fractional_noise(0.02)

# ╔═╡ 3445bdec-e7b2-11ef-2d49-e3c71ef3053d
md"""

#### Exercise 2
Now you need to figure out how to extract the complex visibilities of the data.

!!! hint
	The function in mind is `extract_table`. For an example see an earlier homework.
"""
...
# ╔═╡ 3445be1e-e7b2-11ef-14b4-3bb3044a0461
md"""
...
However, we need to ensure our raster has a positive flux density for each pixel.
Additionally, since image total flux is degenerate with gains we also want to make sure the image is normalized to some fiducial value `ftot` that we will take as given apriori. To satisfy both of these constraints, we will transform our pixel parameters `c` using the softmax function or the inverse of the centered log-ratio transform. 
...
# ╔═╡ 3445be96-e7b2-11ef-1ccf-036c93274894
md"""
...
# ╔═╡ 3445be96-e7b2-11ef-1b1f-c1904afab51f
skymeta = (;ftot = 1.1, mimg = mprior./flux(mprior))

# ╔═╡ 3445beaa-e7b2-11ef-15a3-1f1410ba0c3e
md"""
...
# ╔═╡ 3445beb4-e7b2-11ef-3762-a5ddc48e4363
cprior = corr_image_prior(grid, dvis)

# ╔═╡ 3445bec8-e7b2-11ef-2d36-f1cdfde4d929
md"""
...
# ╔═╡ 3445bed2-e7b2-11ef-1704-33833354aa5b
prior = (
         c = cprior,
...
# ╔═╡ 3445bedc-e7b2-11ef-3559-4d8f4ec085a5
md"""
Now we can construct our sky model. Unlike the previous homework,
we have constant parameters that we need to pass through to `sky`. 
These are supplied by passing the keyword argument `metadata`.
"""

# ╔═╡ 3445bee6-e7b2-11ef-3d00-39317598c46d
skym = SkyModel(sky, prior, grid; metadata=skymeta)

# ╔═╡ 3445bef0-e7b2-11ef-0e2e-d7d1243971a6
md"""
Unlike other imaging examples
(e.g., Imaging a Black Hole using only Closure Quantities) we also need to include
a model for the instrument, i.e., gains.
"""

# ╔═╡ 49000c5f-d3f1-4529-abd6-1034381e4b7a
md"""
For the instrument model, we can use a Single *Jones* matrix, or really just a 1D complex number. This is given by the 
"""

# ╔═╡ db72a39b-4929-4117-b04c-33d24a0dd5a3
begin
	fgain(x) = exp(x.lg + 1im*x.gp)
	G = SingleStokesGain(fgain)
end

# ╔═╡ 193ffe70-fc19-4620-9e6a-146477419c46
md"""
Note that we do not specify the array at all here. This is on purpose since the specifics of the observation, e.g., what stations observed when are abstracted and supplied later. 
"""

# ╔═╡ 7ed30f7b-60a9-4033-b573-f1c48a4f7155
md"""
Now we specify the instrument priors. The gains will be broken into two components
  - Gain amplitudes which are typically known to 10-20%, except for LMT, which has amplitudes closer to 50-100%.
  - Gain phases which are more difficult to constrain and can shift rapidly.

To specify the gains for each site we use the `IIDSitePrior` and `ArrayPrior` composite types. `IIDSitePrior` specifies the prior for a single site and means
that for each time segment, the gain is independent. For the EHT, since the data
is apriori calibrated we use the `ScanSeg` time segmentation, which means that the gain is independent for each *scan*, the other options are:
 - `IntegSeg`: every integration time has an independent intrument parameter
 - `TrackSeg`: The instrument parameter is constant over the whole track.
"""

# ╔═╡ 3445bf04-e7b2-11ef-1a75-b1d24827e754
intpr = (
...
# ╔═╡ 82f6c71f-b013-439e-b52b-7783b8e20bc5
md"""
We can now form the instrument model similarly to the SkyModel
"""

# ╔═╡ defc8fc9-fd5b-40fd-8fc9-b3bcc035202d
intmodel = InstrumentModel(G, intpr)

# ╔═╡ 3445bf22-e7b2-11ef-37bc-73d57797006b
begin
	using Enzyme
	post = VLBIPosterior(skym, intmodel, dvis; admode=set_runtime_activity(Enzyme.Reverse))
end

# ╔═╡ 3445bf68-e7b2-11ef-0326-61eb8f5ef9ac
begin
	using Optimization
...
# ╔═╡ 3445bf90-e7b2-11ef-13fe-f53e82020413
begin
	using CairoMakie
	res = residuals(post, xopt)
	plotfields(res[1], :uvdist, :res)
end

# ╔═╡ 3445c006-e7b2-11ef-1808-530ddba5d06f
begin
	using AdvancedHMC
	chain = sample(rng, post, NUTS(0.8), 1_000; n_adapts=500, progress=false, initial_params=xopt)
end

# ╔═╡ 3445bf18-e7b2-11ef-36b2-2b51c6b4568c
md"""
...
not need `set_runtime_activity`.
"""

# ╔═╡ 3445bf4a-e7b2-11ef-2cdb-035513653c7b
md"""
!!! note
	*Question* How does Comrade deal with the fact that the parameters have a hard boundary? The answer is we transform the model parameters. What function do we use for that? The answer is in a prior tutorial.
"""

# ╔═╡ 3445bf54-e7b2-11ef-25ed-9129476e3e07
md"To initialize our sampler we will use optimize using Adam"

# ╔═╡ 3445bf74-e7b2-11ef-2fbd-39adb63f9c20
md"""
!!! warn
...
# ╔═╡ 3445bf7c-e7b2-11ef-00f5-553a1ec9c65f
md"First we will evaluate our fit by plotting the residuals"

# ╔═╡ 3445bf9a-e7b2-11ef-137f-bb520465d72e
md"""
These look reasonable, although there may be some minor overfitting. This is actually a generic feature of a MAP estimate for these models. In fact, the MAP estimate is especially poor here since it depends on the choice of parameterization. The reason for this is left to an exercise for the reader. However, it has to do with why the posterior density changes as we transform our parameters.
"""

# ╔═╡ 3445bfa2-e7b2-11ef-179c-7bbfde525fc3
begin
...
# ╔═╡ 3445bfb8-e7b2-11ef-11df-9120093dda50
md"""
...
# ╔═╡ 3445bfc2-e7b2-11ef-0d9f-29436eefc6a9
begin
	intopt = instrumentmodel(post, xopt)
	gtp = Comrade.caltable(angle.(intopt))
	plotcaltable(gtp)
end

# ╔═╡ 3445bfe0-e7b2-11ef-017d-4b6ac6d05d93
md"""
...
# ╔═╡ 3445bfea-e7b2-11ef-050a-bb4845bc8ac5
begin
	gta = Comrade.caltable(abs.(intopt))
	plotcaltable(gta)
end

# ╔═╡ 3445bffe-e7b2-11ef-293e-8761c9a15e52
md"""
...
# ╔═╡ 3445c01c-e7b2-11ef-1036-5b4522f14f1c
md"""
!!! note
...
# ╔═╡ 3445c026-e7b2-11ef-030d-f1f4d88ee557
md"Now we prune the adaptation phase"

# ╔═╡ 3445c030-e7b2-11ef-3795-3b12bc526f4e
chainsub = chain[501:end]

# ╔═╡ 3445c03a-e7b2-11ef-3302-bbf33ee92129
md"""
!!! warn
	This should be run for likely an order of magnitude more steps to properly estimate expectations of the posterior.
"""

# ╔═╡ 3445c044-e7b2-11ef-3479-6bf4345f96eb
md"""
...
# ╔═╡ 3445c058-e7b2-11ef-2688-21084410a0f8
md"""
Now let's look at the unceratainty of our gain solutions.
First we create a `caltable` the same way but making sure all of our variables have errors
attached to them. For this, we use the excellent Measurements package, which will do the linear error propagation for you.
"""

# ╔═╡ 3445c078-e7b2-11ef-1966-75200203fd10
md"Now let's plot the phase curves"

# ╔═╡ 3445c080-e7b2-11ef-27e6-0d80b60f88fc
plotcaltable(ctable_ph)

# ╔═╡ 3445c08a-e7b2-11ef-2e08-43647150a734
md"and now the amplitude curves"

# ╔═╡ 3445c094-e7b2-11ef-1aa9-5307e30595db
plotcaltable(ctable_am)

# ╔═╡ 3445c0aa-e7b2-11ef-04a0-4b203db8537c
md"Finally let's construct some representative image reconstructions."

# ╔═╡ 3445c0bc-e7b2-11ef-17e7-311d3442eb48
begin
...
# ╔═╡ 3445c0c6-e7b2-11ef-37bd-a96c02c8ade8
md"""
...
# ╔═╡ 2b2c01c3-86e9-4886-b2f9-23246a546d51
md"""
### Homework
 1. Try changing different hyperparameters in the image prior to see their effect. Namely, the `σimg` prior will likely change the image reconstruction. Additionally, try chaning the order of the GMRF in the `corr_image_prior` see its docstring for more details.
 2. Change the script to do closure-only imaging. Do you get similar results?
 3. Bonus try fitting your own data. What do you need to change in the model specification for both the sky and instrument models.
"""

# ╔═╡ 3445c0d0-e7b2-11ef-2316-c73afaa8b47a
md"""
---
...
manifest_format = "2.0"
project_hash = "4521c746f26dc4f5dbeb86a2b8b55e2784c22227"
